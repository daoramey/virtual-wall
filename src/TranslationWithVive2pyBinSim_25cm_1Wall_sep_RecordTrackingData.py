...
trackingDataFileName1='Tester_01.txt'
trackingDataFile1 = open(os.path.join(script_dir1, trackingDataFileName1), 'a')
now = datetime.datetime.now()
trackingDataFile1.write(str('++++ Tracking 1wall_sep started +++++++++++  ') + str(now.isoformat()) + str('\n'))
	...
	while 1:
		...
		if msvcrt.kbhit():
			...
			if ord(char) == 48:
				...
				trackingDataFile1.write('Chosen Orientation\n')
				now = datetime.datetime.now()
				#yaw=min(positionVectorSubSampled, key=lambda x: abs(x - yaw))
                #yaw=min(positionVectorSubSampled,key=lambda x:abs(x-yaw))
				trackingDataFile1.write(str(now.isoformat()) + str('  Yaw = ')+str(yaw) + str(' ') + str('Filterset ')+str(Filterset)+ str(' ') + str('PosZ ')+str(posZ)+ str('PosX ')+str(posX) + str('\n'))
				trackingDataFile1.write('angleOffSet= '+str(angleOffset)+str('\n'))

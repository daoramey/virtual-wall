...
SettingsFile.write(	'soundfile signals\SherlockHolmesCUT_mono.wav\nblockSize 512\nfilterSize 65536\nfilterList brirs\MeasOBRIR_44100_Room2_totheMiddle_4deg\Filterlist_Meas_OBRIRs44100_Room2middle_row.txt\nenableCrossfading True\nuseHeadphoneFilter False 999 999 999\nloudnessFactor 1')
SettingsFile.close()

# Init file to record tracking data
trackingDataFile = open('trackingdata.txt', 'a')
now = datetime.datetime.now()
trackingDataFile.write(str('++++ scene4_echoloc_Room2middle_separat +++++++++++  ') + str(now.isoformat()) + str('\n'))
